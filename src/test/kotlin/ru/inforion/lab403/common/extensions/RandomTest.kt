...
class RandomTest {
    ...
    private val q = Array(collectionSize, { x.choice { it.weight }})
    private val pq = x.map { item -> q.count { it.id == item.id }.toDouble() / q.count() }

    private val z = Array(collectionSize, { x.choice() })
    private val pz = x.map { item -> z.count { it.id == item.id }.toDouble() / z.count() }

    private val y = Array(collectionSize, { x.choice { 0.0 } })
    private val py = x.map { item -> y.count { it.id == item.id }.toDouble() / z.count() }

    @Test fun choiceBasicTest() {
        assertEquals(x.sumByDouble { it.weight }, 1.0, tolerance)
    }

    @Test fun choiceAccumTest() {
        assertEquals(pq.sum(), 1.0, tolerance)
    }

    @Test fun choiceWeightedEachTest() {
        x.zip(pq).forEach { assertEquals(it.first.weight, it.second, tolerance) }
    }
    ...
    fun error(actual: Int, expected: Int): Double = (actual - expected) / expected.toDouble()

    fun assertHistogram(data: ByteArray, size: Int) {
        val histogram = math.histogram(data)
        val bad = histogram.find { error(it.count, size) > 0.01 }
        assert(bad == null) { "Found bad bin: $bad -> error = ${error(bad!!.count, size)}" }
    }

    @Test
    fun randbytes() {
        assertHistogram(random.randbytes(100000), 100000)
    }
    ...
    fun success() {
        val prob = 0.69
        val total = 1_000_000
        val data = Array(total) { random.success(prob).toInt() }
