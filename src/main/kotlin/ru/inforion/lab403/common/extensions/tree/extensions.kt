 ...
 */
fun <T: Serializable> Node<T>.depth() = backward.count() - 1
 ...
 */
fun <T: Serializable> Node<T>.create(content: T) = Node(content).also { add(it) }

/**
 * Returns list of children (not iterator) to safe remove it all
 */
fun <T: Serializable> Node<T>.listOfChildren() = map { it }

/**
 * Removes and unlinks all children of [this] node
 */
fun <T: Serializable> Node<T>.clear() = listOfChildren().forEach { remove(it) }

/**
 * Adds all [newChildren] to [this] node
 */
fun <T: Serializable> Node<T>.addAll(newChildren: List<Node<T>>) = newChildren.forEach { add(it) }